...
class InfluenceMapping(Object):
    ...
    def calculate(self):
        ...
        if mirror_mode:
            results.append(({infl: infl for infl in self.destinationInfluences}, "回到自己"))
        ...
        if self.config.use_distance_matching:
            ...
            results.append((matches, "距离"))

        if self.config.use_name_matching:
            results.append((nameMatches(self.config.globs, self.influences, self.destinationInfluences, mirror_mode=mirror_mode), "名称"))

        if self.config.use_label_matching:
            results.append((labelMatches(self.influences, self.destinationInfluences, mirror_mode=mirror_mode), "标签"))

        if self.config.use_dg_link_matching:
            matches = dg_matches(self.influences, self.destinationInfluences, self.dg_resolver())
            results.append((matches, "依存关系连接"))
